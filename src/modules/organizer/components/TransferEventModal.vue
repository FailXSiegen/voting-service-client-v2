        ...
        <div class="modal-body">
          ...
          <div v-else>
            <div v-if="event" class="mb-4">
              ...
              <p class="mb-1">
                <strong>{{ $t('view.organizers.events.title') }}:</strong> {{ event.title }}
              </p>
              <p class="mb-0">
                <strong>{{ $t('view.organizers.events.currentOwner') }}:</strong>
                {{ event.organizer?.username }} ({{ event.organizer?.email }})
              </p>
            </div>

            <div v-if="organizers.length > 0" class="form-group">
              <label for="targetOrganizer" class="form-label">{{
                $t('view.organizers.events.transfer.selectTarget')
              }}</label>
              <select
                id="targetOrganizer"
              ...
              >
                <option value="" disabled selected>
                  {{ $t('view.organizers.events.transfer.selectPlaceholder') }}
                </option>
                <option
...
const availableOrganizers = computed(() => {
  ...
  return organizers.value.filter((organizer) => organizer.id !== props.event?.organizer?.id);
});
...
const {
  ...
  onResult,
  onError,
} = useLazyQuery(ORGANIZERS, null, { fetchPolicy: 'network-only' });

// Transfer event mutation
const {
  mutate: transferEventMutation,
  onDone,
  onError: onTransferError,
} = useMutation(TRANSFER_EVENT);

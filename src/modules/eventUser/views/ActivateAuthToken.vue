  ...
  <CorePageLayout>
    ...
    >
      <h3
        ...
        v-html="$t('view.activateAuthToken.subTitle', { eventTitle: event.title })"
      ></h3>
      <p class="description mb-4" v-html="$t('view.activateAuthToken.description')"></p>
...
fetchEventById(eventId).then((response) => {
  ...
  if (
    (response?.publicnameReadonly === true || response?.publicnameReadonly === 1) &&
    publicNameFromQuery
  ) {

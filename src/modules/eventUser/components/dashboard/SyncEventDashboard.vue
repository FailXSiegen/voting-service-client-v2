<template>
  ...
  <template v-if="eventUser && eventUser.verified">
    ...
    <PollStatus
      ...
      :vote-amount="eventUser?.voteAmount || 0"
    />
    ...
    <AlertBox v-if="!showMoreEnabled" type="info" :message="$t('view.results.noMoreResults')" />
    <PollModal
      ...
      @submit="onSubmitPoll"
    />
    <ResultModal ref="resultModal" :poll-result="lastPollResult" :event="event" />
  </template>
</template>
...
const showVotingModal = computed(() => {
  ...
  console.log(
    `[DEBUG:VOTING] showVotingModal berechnet: hasVotesAvailable=${hasVotesAvailable}, isAllowedToVote=${isAllowedToVote}, usedVotes=${usedVotes}, maxVotes=${maxVotes}, hasRemainingVotes=${hasRemainingVotes}, RESULT=${result}`
  );
...
watch(
  ...
  (newVal, oldVal) => {
    ...
    console.log(
      `[DEBUG:VOTING] 🔍 eventUser.voteAmount=${eventUser.value?.voteAmount}, eventUser.allowToVote=${eventUser.value?.allowToVote}`
    );
    ...
    if (newVal >= maxVotes && maxVotes > 0) {
      console.log(
        `[DEBUG:VOTING] 🎯 Alle Stimmen abgegeben (${newVal}/${maxVotes}), setze isSubmitting zurück`
      );
      ...
      if (pollModal.value?.resetSubmittingState) {
        try {
          ...
          console.log(
            '[DEBUG:VOTING] ✅ isSubmitting erfolgreich zurückgesetzt via resetSubmittingState'
          );
        } catch (e) {
  ...
  watchdogInterval = setInterval(() => {
    try {
      ...
      if (
        ...
        if (pollModal.value && poll.value && !poll.value.closed && pollState.value !== 'closed') {
...
activePollEventUserQuery.onResult(({ data }) => {
  ...
  if (pollStatePersistence.canVote(poll.value.id, eventUser.value, props.event)) {
    ...
    if (eventUser.value?.id && poll.value?.id) {
      ...
      userVoteCycleQuery.onResult(({ data }) => {
        if (data?.userVoteCycle) {
          ...
          const newStoredCounter = pollStatePersistence.restoreVoteCounter(
            poll.value.id,
            props.event.id
          );
          ...
          if (newStoredCounter !== voteCounter.value) {
            console.warn(
              `[DEBUG:COUNTER] Problem beim Speichern! Expected=${voteCounter.value}, Got=${newStoredCounter}`
            );
          }
            ...
            setTimeout(() => {
              ...
              if (pollModal.value) {
                ...
                if (serverVoteCycle === 0) {
                  ...
                  console.log(
                    '[DEBUG:VOTING] Neuer Poll (activePollEventUser): Formular wird zurückgesetzt'
                  );
                } else {
                  console.log(
                    `[DEBUG:VOTING] Split-Voting (activePollEventUser, Zyklus ${serverVoteCycle}): Formular wird NICHT zurückgesetzt`
                  );
                }
                ...
                setTimeout(() => {
                  ...
                  if (pollModal.value) {
                    ...
                    console.warn(
                      '[DEBUG:VOTING] pollModal.value wurde null zwischen reset und showModal!'
                    );
                  }
                }, 50);
              } else {
                console.warn(
                  '[DEBUG:VOTING] pollModal.value ist null beim Versuch, reset aufzurufen und Modal zu öffnen'
                );
              }
            }, 100);
          } else {
          ...
          const storedCounter = pollStatePersistence.restoreVoteCounter(
            poll.value.id,
            props.event.id
          );
          ...
          if (voteCounter.value > 99000) {
            console.warn(
              `[DEBUG:COUNTER] Unplausibel hoher voteCounter (${voteCounter.value})! Setze auf ${storedUsedVotes + 1}`
            );
            ...
            setTimeout(() => {
              ...
              if (pollModal.value) {
                ...
                if (currentUsedVotes === 0) {
                  pollModal.value.reset(false);
                  console.log(
                    '[DEBUG:VOTING] Neuer Poll (activePollEventUser Fallback 1): Formular wird zurückgesetzt'
                  );
                } else {
                  console.log(
                    `[DEBUG:VOTING] Split-Voting (activePollEventUser Fallback 1, ${currentUsedVotes} verwendet): Formular wird NICHT zurückgesetzt`
                  );
                }
      ...
      // Synchronisiere den verwendeten Stimmen-Zähler mit dem gespeicherten Zustand
      // Dies ist wichtig, um nach einem Reload den korrekten Stimmenzähler wiederherzustellen
      votingProcess.usedVotesCount.value = pollStatePersistence.getUsedVotes(
        poll.value.id,
        props.event.id
      );
      ...
      if (voteCounter.value <= totalAllowedVotes) {
        ...
        setTimeout(() => {
          ...
          if (currentUsedVotes === 0) {
            pollModal.value?.reset(false);
            console.log(
              '[DEBUG:VOTING] Neuer Poll (activePollEventUser Fallback 2): Formular wird zurückgesetzt'
            );
          } else {
            console.log(
              `[DEBUG:VOTING] Split-Voting (activePollEventUser Fallback 2, ${currentUsedVotes} verwendet): Formular wird NICHT zurückgesetzt`
            );
          }
  ...
  if (data.pollResult.length > 0) {
    pollResults.value = [
      ...pollResults.value.filter(
        (existing) => !data.pollResult.some((newResult) => newResult.id === existing.id)
      ),
      ...data.pollResult,
...
const pollLifeCycleSubscription = useSubscription(POLL_LIFE_CYCLE_SUBSCRIPTION, {
  eventId: props.event.id,
});
pollLifeCycleSubscription.onResult(async ({ data }) => {
  ...
  if (pollClosedEventReceived.value) {
    console.warn(
      '[DEBUG:VOTING] Poll ist bereits als geschlossen markiert, ignoriere weiteres Life-Cycle-Event'
    );
    return;
  }
  ...
  if (data.pollLifeCycle.state === 'closed') {
    ...
    try {
      ...
      apolloClient
        ...
        .then((result) => {
          if (result.data?.pollResult && result.data.pollResult.length > 0) {
            ...
            try {
              ...
              console.error(
                '[DEBUG:VOTING] Fehler beim direkten Anzeigen des ResultModals:',
                modalError
              );
            }
          }
        ...
        .catch((e) => {
          console.error(
            '[DEBUG:VOTING] SOFORTAKTION: Fehler beim sofortigen Laden der Ergebnisse:',
            e
          );
        });
    } catch (e) {
  ...
  if (isProcessingLifeCycleEvent.value) {
    ...
    if (isProcessingLifeCycleEvent.value) {
      console.warn(
        '[DEBUG:VOTING] Life-Cycle-Event wird immer noch verarbeitet, ignoriere dieses Event'
      );
      return;
    }
  }
  ...
  try {
    ...
    if (poll.value && poll.value.id) {
      window._currentActivePollId = poll.value.id;
      console.warn(
        `[DEBUG:VOTING] Aktuelle globale Poll-ID aktualisiert: ${window._currentActivePollId}`
      );
    } else {
    ...
    if (pollState.value === 'new') {
      ...
      if (resultModal.value) {
        try {
          ...
          setTimeout(() => {
            if (
              resultModal.value &&
              resultModal.value.isVisible &&
              resultModal.value.isVisible.value
            ) {
              resultModal.value.hideModal();
      ...
      setTimeout(() => {
        ...
        const timeoutInSplitVoteSituation =
          timeoutUsedVotes > 0 && timeoutUsedVotes < timeoutMaxVotes;
      ...
      if (showVotingModal.value) {
        ...
        if (eventUser.value?.id && poll.value?.id) {
          ...
          if (!eventUserId || !pollId) {
            console.error('Ungültige Parameter für userVoteCycle (neue Poll):', {
              eventUserId,
              pollId,
            });
            return;
          ...
          userVoteCycleQuery.onResult(({ data }) => {
            if (data?.userVoteCycle) {
              ...
              const persistedUsedVotes = pollStatePersistence.getUsedVotes(
                poll.value.id,
                props.event.id
              );
              ...
              if (finalVoteCycle > currentClientValue) {
                console.log(
                  `[DEBUG:VOTING] Server hat höheren Wert (${finalVoteCycle} > ${currentClientValue}), übernehme Server-Wert`
                );
                votingProcess.usedVotesCount.value = finalVoteCycle;
              } else if (finalVoteCycle < currentClientValue) {
                console.warn(
                  `[DEBUG:VOTING] ⚠️ Server-Wert (${finalVoteCycle}) ist niedriger als Client-Wert (${currentClientValue}), behalte Client-Wert`
                );
              ...
              setTimeout(() => {
                ...
                console.log(
                  `[DEBUG:VOTING] Modal-Öffnungsprüfung (pollLifeCycle): maxVotes=${maxVotes}, serverVoteCycle=${serverVoteCycle}, clientUsedVotes=${clientUsedVotes}`
                );
                ...
                if (maxVotes > 0 && serverVoteCycle < maxVotes && clientUsedVotes < maxVotes) {
                  ...
                  if (resultModal.value) {
                    try {
                      ...
                      console.error(
                        '[DEBUG:VOTING] Fehler beim Schließen des ResultModals:',
                        modalError
                      );
                    }
                  }
                  ...
                  if (serverVoteCycle === 0) {
                    ...
                    console.log(
                      `[DEBUG:VOTING] Split-Voting (pollLifeCycle, Zyklus ${serverVoteCycle}): Formular wird NICHT zurückgesetzt`
                    );
                  }
                  ...
                  setTimeout(() => {
                    ...
                    console.log(
                      `[DEBUG:VOTING] Finale Prüfung vor showModal (pollLifeCycleSubscription): finalUsedVotes=${finalUsedVotes}, finalMaxVotes=${finalMaxVotes}`
                    );

                    if (finalUsedVotes >= finalMaxVotes && finalMaxVotes > 0) {
                      console.log(
                        `[DEBUG:VOTING] ABBRUCH: Alle Stimmen bereits abgegeben (${finalUsedVotes}/${finalMaxVotes}), Modal wird NICHT geöffnet`
                      );
                      return;
                    }

                    console.log(
                      '[DEBUG:VOTING] >>> Rufe pollModal.showModal() auf (pollLifeCycleSubscription)'
                    );
                    pollModal.value?.showModal();
                  }, 250);
                } else {
              ...
              setTimeout(() => {
                ...
                if (maxVotes > 0 && clientUsedVotes < maxVotes) {
                  ...
                  if (clientUsedVotes === 0) {
                    pollModal.value?.reset(false); // Vollständiges Zurücksetzen erzwingen
                    console.log(
                      '[DEBUG:VOTING] Neuer Poll (Fallback): Formular wird zurückgesetzt'
                    );
                  } else {
                  ...
                  setTimeout(() => {
                    ...
                    const finalMaxVotes = maxVotes;
                    console.log(
                      `[DEBUG:VOTING] Finale Prüfung vor showModal (Fallback 1): finalUsedVotes=${finalUsedVotes}, finalMaxVotes=${finalMaxVotes}`
                    );

                    if (finalUsedVotes >= finalMaxVotes && finalMaxVotes > 0) {
                      console.log(
                        `[DEBUG:VOTING] ABBRUCH (Fallback): Alle Stimmen bereits abgegeben (${finalUsedVotes}/${finalMaxVotes}), Modal wird NICHT geöffnet`
                      );
                      return;
                    }
          ...
          setTimeout(() => {
            ...
            if (maxVotes > 0 && clientUsedVotes < maxVotes) {
              ...
              setTimeout(() => {
                ...
                const finalMaxVotes = maxVotes;
                console.log(
                  `[DEBUG:VOTING] Finale Prüfung vor showModal (Fallback 2): finalUsedVotes=${finalUsedVotes}, finalMaxVotes=${finalMaxVotes}`
                );

                if (finalUsedVotes >= finalMaxVotes && finalMaxVotes > 0) {
                  console.log(
                    `[DEBUG:VOTING] ABBRUCH (Fallback 2): Alle Stimmen bereits abgegeben (${finalUsedVotes}/${finalMaxVotes}), Modal wird NICHT geöffnet`
                  );
                  return;
                }
      ...
      if (pollClosedEventReceived.value) {
        console.warn(
          '[DEBUG:VOTING] Poll wurde bereits als geschlossen markiert, ignoriere weiteres Event'
        );
        return;
      }
          ...
          try {
            ...
            setTimeout(() => {
              try {
                ...
                if (window._currentClosureId !== closureId) {
                  console.warn(
                    '[DEBUG:VOTING] Ein neuerer Closure-Prozess wurde gestartet, breche Modal-Anzeige ab'
                  );
                  window._isHandlingPollClosure = false;
                  return;
                }
                ...
                if (pollModal.value) {
                  try {
                    ...
                    console.error(
                      '[DEBUG:VOTING] Fehler beim Schließen des PollModals vor Ergebnisanzeige:',
                      e
                    );
                  }
                }
...
pollAnswerLifeCycleSubscription.onResult(async ({ data }) => {
  ...
  if (poll.value && poll.value.closed) {
    console.warn(
      '[DEBUG:VOTING] Poll ist geschlossen, aber Subscription-Event empfangen. Setze closed-Flag und ignoriere Event.'
    );
...
function onShowMorePollResults() {
  ...
  pollResultsQuery.fetchMore({
    ...
    updateQuery: (previousResult, { fetchMoreResult }) => {
      if (!fetchMoreResult?.pollResult || fetchMoreResult.pollResult.length < pageSize.value) {
        showMoreEnabled.value = false;
...
async function onSubmitPoll(pollFormData) {
  try {
    ...
    if (votingProcess.pollFormSubmitting.value === true) {
      console.warn(
        '[DEBUG:VOTING] pollFormSubmitting ist nach dem Reset immer noch true! Setze explizit zurück.'
      );
      votingProcess.pollFormSubmitting.value = false;
    }
    ...
    if (votingProcess.isProcessingVotes.value === true) {
      console.warn(
        '[DEBUG:VOTING] isProcessingVotes ist nach dem Reset immer noch true! Setze explizit zurück.'
      );
      votingProcess.isProcessingVotes.value = false;
    }
    ...
    if (votingProcess.currentlyProcessingBatch.value === true) {
      console.warn(
        '[DEBUG:VOTING] currentlyProcessingBatch ist nach dem Reset immer noch true! Setze explizit zurück.'
      );
      votingProcess.currentlyProcessingBatch.value = false;
    }
    ...
    currentPollSubmissionId.value =
      'poll-' + currentTimestamp + '-' + Math.random().toString(36).substring(2, 10);
    ...
    try {
      ...
      if (!success) {
        ...
        if (typeof window !== 'undefined' && window._pollClosedDuringVoting === true) {
          ...
          toast(
            `Poll wurde während der Abstimmung geschlossen. ${successfulVotes} Stimme(n) wurden gezählt.`,
            { type: 'info' }
          );
          // Flags zurücksetzen
      ...
      if (typeof window !== 'undefined') {
        try {
          ...
          if (
            !window._lastSyncDashboardEventTimestamp ||
            now - window._lastSyncDashboardEventTimestamp > minTimeBetweenEvents
          ) {
          ...
          console.error(
            '[DEBUG:VOTING] Fehler beim Auslösen des globalen voting:complete Event:',
            e
          );
        }
      }
      ...
      setTimeout(() => {
        ...
        if (pollModal.value) {
          ...
          if (typeof pollModal.value.resetSubmittingState === 'function') {
            try {
              ...
              console.error(
                '[DEBUG:VOTING] Fehler beim Aufruf von resetSubmittingState im Timeout:',
                e
              );
            }
          }
        ...
        if (typeof window !== 'undefined') {
          try {
            ...
            if (
              !window._lastTimeoutEventTimestamp ||
              now - window._lastTimeoutEventTimestamp > minTimeBetweenEvents
            ) {
              // Event-Timestamp aktualisieren
            ...
            console.error(
              '[DEBUG:VOTING] Fehler beim Auslösen des globalen voting:complete Event im Timeout:',
              e
            );
          }
        }
...
function resetUIAfterSubmission() {
  ...
  if (pollModal?.value?.resetSubmittingState) {
    try {
      ...
      console.error(
        '[DEBUG:VOTING] Fehler beim Aufruf von resetSubmittingState in resetUIAfterSubmission:',
        e
      );
    }
  }
  ...
  try {
    ...
    const allVotesUsed =
      (totalAllowedVotes > 0 && usedVotes >= totalAllowedVotes) ||
      votingProcess.votingFullyCompleted.value;
    const partialVotesUsed =
      usedVotes > 0 && usedVotes < totalAllowedVotes && !votingProcess.votingFullyCompleted.value;

    // Stimmzähler korrigieren, falls durch Reload mehr Stimmen gezählt wurden als maximal erlaubt
    if (usedVotes > totalAllowedVotes) {
      console.warn(
        `[DEBUG:VOTING] UI-Reset: Stimmenzähler korrigiert: ${usedVotes} auf ${totalAllowedVotes}`
      );
    ...
    if (allVotesUsed) {
      ...
      if (pollModal.value) {
        ...
        setTimeout(() => {
          ...
          if (false && pollModal.value) {
            // DEAKTIVIERT durch if (false)
            pollModal.value.reset(false);
                    ...
                    try {
                      ...
                      if (pollModal.value) {
                        ...
                        if (
                          ...
                          (!pollModal.value.bootstrapModal || !pollModal.value.modal)
                        ) {
                          try {
                            ...
                            console.error(
                              '[DEBUG:VOTING] Fehler bei Modal-Neuinitialisierung:',
                              initErr
                            );
                          }
                        }
                        ...
                        console.error(
                          '[DEBUG:VOTING] Modal-DOM-Element nicht verfügbar - pollModal.value ist null'
                        );
                      }
                    } catch (modalErr) {
    ...
    if (false && partialVotesUsed) {
      ...
      setTimeout(() => {
        ...
        if (
          ...
          console.warn(
            `[DEBUG:VOTING] NOTFALL-WIEDERERÖFFNUNG: Modal wurde nicht korrekt geöffnet`
          );
          ...
          setTimeout(() => {
            try {
              ...
              try {
                ...
                console.error(
                  '[DEBUG:VOTING] Fehler beim Aufräumen vor Notfalleröffnung:',
                  cleanupErr
                );
              }
              ...
              console.error(
                '[DEBUG:VOTING] Auch Notfall-Wiedereröffnung fehlgeschlagen:',
                emergencyErr
              );
            }
          }, 200);
      ...
      if (
        ...
        setTimeout(() => {
          try {
            ...
            console.error(
              '[DEBUG:VOTING] Fehler-Notfall-Wiedereröffnung fehlgeschlagen:',
              emergencyErr
            );
          }
        }, 500);

...
const { loginAsOrganizer, createAndStartPoll } = require('../lib/liveLoginOrganizer'); // Verwende die spezielle Live-Version
const { loginAsUser } = require('../lib/liveLoginUser');
...
test.describe('Live testing mit Benutzern aus CSV', () => {
  ...
  const batchCount = 2; // Reduziert: 2 Batches
  ...
  for (let testId = 1; testId <= batchCount; testId++) {
    test(`User batch ${testId} votes in poll`, async ({ browser }) => {
      ...
      const usersPerBatch = Math.ceil(totalUsers / batchCount);
      const startIndex = (testId - 1) * usersPerBatch;
      const endIndex = Math.min(startIndex + usersPerBatch, totalUsers);
      const batchUsers = users.slice(startIndex, endIndex);

      console.log(`[Test ${testId}] Batch mit ${batchUsers.length} Benutzern (Indizes ${startIndex} bis ${endIndex - 1} von insgesamt ${totalUsers})`);

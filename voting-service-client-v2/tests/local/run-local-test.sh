...
echo "Connecting to: http://local.digitalwahl.org"
echo "Event ID: 1007, Organizer: herrmann"
echo ""

# Run the test with the live configuration
npx playwright test tests/local/local-test.spec.js --config=playwright.config.js --workers=4

# Check exit status
if [ $? -eq 0 ]; then
    echo "local test completed successfully!"
else
    echo "local test encountered errors. Check the results."
fi
